...
func (self *Logistics) BuildTask(dboper db.DBOperIO, taskid interface{}, curtime int64) string {
	logisticssTaskModel := GetModel(LOGISTICS_TASK)
	if taskid == nil {
		return ""
	}
...
func (self *Logistics) AddList(dboper db.DBOperIO, logicsarr []ErpShipdata, taskid string) error {
	//ordermodel := GetModel(SHOP_ORDER).(*ShopOrder)
...
type Ship100RespData struct {
	Message string                `json:"message"` //错误信息 正确:ok
	State   string                `json:"state"`   //订单状态
	Status  string                `json:"status"`  //状态 正确：200  201
	Data    []LogicstaticSendData `json:"data"`
}
...
func (self *Logistics) GetLogicsInfo(dboper db.DBOperIO, id interface{}) (sendmsg []LogicstaticSendData, status int, err error, idinfo map[string]interface{}) {
	...
	if overseaOk == true {
		...
		if company != "" && shipnum != "" {
			...
			req := httplib.Get(fmt.Sprintf("https://www.kuaidi100.com/query?type=%s&postid=%s", company, shipnum))
			var respdata []byte
			...
			if getData.Status != "200" {
				err = errors.New(getData.Message)
				return
			}
			datalen := len(getData.Data)
			for i := datalen - 1; i >= 0; i-- {
				if i == datalen-1 {
					if overseainfo != "" {
						...
						var itemdata = LogicstaticSendData{Time: getData.Data[i].Time, Context: fmt.Sprintf("包裹清关成功!现已转发国内快递:%s 单号:%s", companyname, shipnum)}
						sendmsg = append(sendmsg, itemdata)
					}
				}
				sendmsg = append(sendmsg, getData.Data[i])
			}
			status, err = strconv.Atoi(getData.State)
			if err != nil {
