...
func (self *LoginController) loginSucesss(userinfo db.Params, changeinfo map[string]interface{}, noreturn bool) {
	...
	senddata["uid"] = id
	_, err = self.GetDb().Raw(fmt.Sprintf(`update  %s set %s  where id=?`, userModel.TableName(), db.SqlGetKeyValue(changeinfo, "=")), id).Exec()
	if err == nil {
		userModel.GetInfoAndCache(self.GetDb(), id, true) //更新缓存
		if noreturn == false {
...
func (self *LoginController) addUser(adddata map[string]interface{}, fieldkey string, fieldvalue string, noreturn bool) db.Params {
	...
	num, err := self.GetDb().Raw(fmt.Sprintf(`select * from %s where id=? limit 1`, userGroupModel.TableName()), membergroupid).Values(&res)
	if err != nil && num == 0 {
	...
	_, err = self.GetDb().Raw(fmt.Sprintf("insert into %s (%s) values (%s)", userModel.TableName(), keys, values)).Exec()
	if err != nil {
		self.AjaxReturnError(errors.WithStack(err))
	}

	num, err = self.GetDb().Raw(fmt.Sprintf(`select * from %s where %s=? limit 1`, userModel.TableName(), fieldkey), fieldvalue).Values(&res)
	if err == nil && num > 0 {
...
func (self *LoginController) Login() {
	...
	userModel := models.GetModel(names.USER)
	num, err := self.GetDb().Raw(fmt.Sprintf(`select * from %s where account=? and password=? limit 1`, userModel.TableName()), data.Username, passMd5).Values(&res)
	if err == nil && num > 0 {
...
func (self *LoginController) LoginOut() {
	...
	// o := orm.NewOrm()
	userModel := models.GetModel(names.USER)
	_, err := self.GetDb().Raw(fmt.Sprintf(`update %s set user_token='%s',token_expire='%d' where id='%s'`, userModel.TableName(), "", 0, self.GetUid())).Exec()
	if err == nil {
		userModel.GetInfoAndCache(self.GetDb(), self.GetUid(), true) //更新缓存
...
func (self *LoginController) LoginWithPhone() {
	...
	var res []db.Params
	// o := orm.NewOrm()
	num, err := self.GetDb().Raw(fmt.Sprintf(`select * from %s where phone=? limit 1`, userModel.TableName()), data.Phone).Values(&res)
	if err == nil {
		//是老用户
		if num > 0 {
			logs.Info("old user")
...
func (self *LoginController) WchatLoginCallback() {
	...
	var res []db.Params
	// o := orm.NewOrm()
	num, err := self.GetDb().Raw(fmt.Sprintf(`select * from %s where wchat_unionid=? limit 1`, userModel.TableName()), userInfo.Unionid).Values(&res)
	var uid string
	var token string
